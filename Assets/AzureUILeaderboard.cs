...
public class AzureUILeaderboard : MonoBehaviour
{
    ...
    public void QueryListLevel(string level)
    {
        _leaderboardItems.Clear();
        lstId.Clear();
        azure.Where<pandafruitfarm>(p => p.Level == level, QueryListLevelCallback);
    }
    ...
    {
        _leaderboardItems.Clear();
        azure.Where<pandafruitfarm>(p => p.Username != null, ReadHandler);
    }

    public void ReadHandler(AzureResponse<List<pandafruitfarm>> response)
    {
        ...
        foreach (var item in list)
        {
            ...
            //getDataFacebook.GetAvatar(item.IdFacebook, 1);
